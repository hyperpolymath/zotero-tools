...
# setup.sh — Universal setup script for zotero-tools
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/zotero-tools/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
main() {
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/zotero-tools.git"
        info "Then: cd zotero-tools && ./setup.sh"
        exit 1
    fi

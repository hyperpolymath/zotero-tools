...
* *Type of vulnerability* (e.g., XSS, SQL injection, improper input
validation)
...
* *Citation data validation*: All citation fields validated with Zod
schemas
...
* *User data stays local*: Validation results never leave the user’s
machine
...
[arabic]
. *Keep Zotero updated*: Use latest stable version
. *Review exported data*: Inspect JSON before importing to Fogbinder
. *Use HTTPS for Zotero sync*: If using Zotero cloud sync
. *Backup your library*: Regular backups of Zotero data
...
* https://www.zotero.org/support/security[Zotero Security Policy]
* https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/Security_best_practices[WebExtension
Security]

...
// Node types in the epistemic network
type nodeType =
...
let buildFromAnalysis = (
  ...
  // Add source nodes
  let withSources = Js.Array2.reduce(sources, (acc, source) => {
    addNode(
      acc,
      {
        id: source,
  ...
  // Add mystery nodes
  let withMysteries = Js.Array2.reduce(mysteries, (acc, mystery) => {
    addNode(
      acc,
      {
        id: mystery.content,
